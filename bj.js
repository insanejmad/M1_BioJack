...
//deck of cards : name, alt, src and value
...
function SetDraw(){
  ...
  img.alt=alternates[num];
        /*
        cards.splice(num,1);
        sources.splice(num,1);
        alternates.splice(num,1);
        values.splice(num,1);

        This three command lines worked fine, and let us remove drawn card from our deck to not draw the same one twice. Problem is, we did not manage to reset the deck in our "Reset" function, and chose to removes these to have a better chance at playing a random unbiased game.
        */
  document.getElementById('pcd1').appendChild(img);
  let num3 = Math.floor(Math.random()*sources.length);
  ...
  //dealer cards
  let num2 = Math.floor(Math.random()*sources.length);
  var dt = values[num2];
  document.getElementById("dealerTot").innerHTML=dt;
